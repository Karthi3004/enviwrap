  ...
  return (
    <div className="p-4 sm:p-6 max-w-5xl mx-auto">
      <div className="mb-5">
        ...
        <p className="text-stone-500 text-xs mt-0.5">Enviwrap dMRV · VM0042 v2.2 · Tamil Nadu</p>
      </div>
      ...
      <div className="bg-white border border-stone-200 rounded-2xl p-4 mb-3">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h2 className="text-sm font-semibold text-stone-900">VVB Readiness</h2>
        </div>
        {vvb ? (
      ...
      <div className="bg-white border border-stone-200 rounded-2xl p-4 mb-3">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-stone-900">Satellite Alerts</h2>
          </div>
          <button onClick={() => navigate('/qaqc')} className="text-xs text-emerald-700">View all →</button>
        </div>
        {alerts.length === 0 ? (
          <div className="text-center py-5">
            <CheckCircle2 size={24} className="text-emerald-500 mx-auto mb-1.5" />
            <p className="text-xs text-stone-500">No unresolved discrepancies</p>
          </div>
        ) : (
          <div className="space-y-2">
            {alerts.slice(0, 5).map((alert) => (
              <div key={alert.id} className="flex items-start gap-2 p-2.5 bg-stone-100/70 rounded-xl">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-stone-500 mt-0.5">{alert.farms?.farm_id}</p>
                </div>
                <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium flex-shrink-0 ${
                  alert.severity === 'BLOCK' ? 'bg-red-500/20 text-red-700' :
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <div className="bg-white border border-stone-200 rounded-2xl p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="text-sm font-semibold text-stone-900">Visits Due</h2>
            {visits?.count > 0 && (
              <span className="ml-auto text-[10px] bg-blue-500/20 text-blue-700 px-2 py-0.5 rounded-full">{visits.count}</span>
            )}
          </div>
          {!visits?.due_visits?.length ? (
            <p className="text-xs text-stone-500 text-center py-3">All farms visited recently</p>
          ) : (
            <div className="space-y-2">
              {visits.due_visits.slice(0, 4).map((farm) => (
                <div key={farm.id} className="flex items-center justify-between">
                  <div>
                    <p className="text-xs font-medium text-stone-800">{farm.farmer_full_name}</p>
                    <p className="text-[10px] text-stone-500">{farm.farm_id}</p>
                  </div>
                  <button onClick={() => navigate(`/farms/${farm.id}`)} className="text-xs text-emerald-700">Visit →</button>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="bg-white border border-stone-200 rounded-2xl p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="text-sm font-semibold text-stone-900">Lab Samples</h2>
          </div>
          {lab ? (
            <div>
              <div className="grid grid-cols-2 gap-2 mb-3">
                <div className="bg-stone-100/70 rounded-xl p-3 text-center">
                  <div className="text-xl font-bold text-stone-900">{lab.pending?.length || 0}</div>
                  <div className="text-[10px] text-stone-500">Pending</div>
                </div>
                <div className="bg-stone-100/70 rounded-xl p-3 text-center">
                  <div className="text-xl font-bold text-emerald-700">{lab.received?.length || 0}</div>
                  <div className="text-[10px] text-stone-500">Received</div>
                </div>
              </div>
              {lab.pending?.slice(0, 2).map((s) => (
                <div key={s.id} className="flex items-center gap-2 py-1.5 border-t border-stone-200">
                  <div className="w-1.5 h-1.5 bg-amber-400 rounded-full flex-shrink-0" />
                  <span className="text-xs text-stone-600 truncate">{s.farms?.farm_id || s.farm_id}</span>
                </div>
              ))}
            </div>
