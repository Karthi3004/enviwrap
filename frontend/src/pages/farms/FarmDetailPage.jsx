...
export default function FarmDetailPage() {
  ...
  const modules = [
    { label: 'Baseline Data', icon: ClipboardList, path: `/farms/${id}/baseline`, desc: 'Module 2 — 3-year history', color: 'text-blue-400', bg: 'bg-blue-500/5 border-blue-500/20' },
    { label: 'SOC Samples', icon: FlaskConical, path: `/farms/${id}/soc`, desc: 'Module 3 — Soil measurements', color: 'text-purple-400', bg: 'bg-purple-500/5 border-purple-500/20' },
    { label: 'Monitoring', icon: Activity, path: `/farms/${id}/monitoring`, desc: 'Module 4 — Annual visits', color: 'text-emerald-400', bg: 'bg-emerald-500/5 border-emerald-500/20' },
  ];
  ...
  return (
    <div className="p-4 sm:p-6 max-w-3xl mx-auto">
      ...
      <div className="flex items-start gap-3 mb-5">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <h1 className="text-lg font-bold text-white">{farm.farmer_full_name}</h1>
            <span className="font-mono text-xs text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded flex-shrink-0">{farm.farm_id}</span>
          </div>
          <p className="text-gray-500 text-xs mt-0.5">{farm.village}, {farm.block_taluk}, {farm.district}</p>
        </div>
        <button onClick={runQAQC} disabled={runningQAQC}
          className="flex items-center gap-1 text-xs text-gray-400 border border-gray-700 px-2.5 py-1.5 rounded-lg flex-shrink-0">
      ...
      {/* Farm info grid */}
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-4 mb-3">
        <h2 className="text-xs font-semibold text-gray-400 mb-3 uppercase tracking-wide">Farm Details</h2>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-4 mb-4">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h2 className="text-sm font-semibold text-white">QA/QC</h2>
          {unresolvedFlags.length === 0 && <CheckCircle2 size={14} className="text-emerald-400 ml-auto" />}
        </div>
        {unresolvedFlags.length === 0 ? (
          <p className="text-xs text-gray-500 text-center py-2">All checks passed</p>
        ) : (
          <div className="space-y-2">
            ...
            {blockCount > 0 && (
              <p className="text-xs text-red-400 text-center pt-1">⚠ {blockCount} blocker(s) — resolve before VVB submission</p>
            )}
          </div>
        )}
      </div>
      ...
      {/* Module cards */}
      <h2 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Data Entry Modules</h2>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {modules.map(({ label, icon: Icon, path, desc, color, bg }) => (
          <button key={label} onClick={() => navigate(path)}
            className={`border rounded-2xl p-4 text-left transition-all active:scale-95 hover:border-gray-600 ${bg} border-gray-800`}>
